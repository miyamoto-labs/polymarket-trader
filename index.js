...
async function initClient() {
  try {
    ...
    clientReady = true;
    console.log('ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Client initialized successfully');
  } catch (err) {
    initError = err.message;
    console.error('ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Client initialization failed:', err.message);
  }
}
...
async function postOrderViaCurl(signedOrder, orderType = 'GTC') {
  ...
  console.log('ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡ Posting order via curl...');
  const result = execSync(curlCmd, { encoding: 'utf8', timeout: 15000 });
  console.log('ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡ Curl response:', result.substring(0, 200));
...
app.post('/bet', auth, async (req, res) => {
  try {
    ...
    console.log(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Placing order: ${side} ${roundedSize} shares @ ${roundedPrice}`);
    ...
    try {
      ...
      console.log(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Curl failed (${curlErr.message}), trying native...`);
      postResult = await client.postOrder(signedOrder);
    }
    console.log(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Order result: ${JSON.stringify(postResult)}`);
    res.json({
    ...
    console.error(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Order failed: ${err.message}`);
...
app.post('/sign-order', auth, async (req, res) => {
  try {
    ...
    console.log(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Order signed. Price: ${roundedPrice}, Size: ${roundedSize}, Side: ${side}`);
    res.json({
      success: true,
    ...
    console.error(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Sign failed: ${err.message}`);
...
app.post('/forward-order', auth, async (req, res) => {
  try {
    ...
    const hArgs = Object.entries(l2Headers).map(([k, v]) => "-H '" + k + ": " + v + "'").join(' ');
    const curlCmd = 'curl -s -w "\n%{http_code}" -X POST https://clob.polymarket.com/order ' + hArgs + " -H 'Content-Type: application/json' -d @" + tmpFile;
    const result = execSync(curlCmd, { timeout: 15000, encoding: 'utf8' });
...
app.listen(PORT, async () => {
  console.log(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Polymarket Trader running on port ${PORT}`);
  await initClient();
});
